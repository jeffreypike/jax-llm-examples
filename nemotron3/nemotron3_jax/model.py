...
def rms_norm(x: jax.Array, gamma: jax.Array | None, eps: jax.Array | float, axis=-1) -> jax.Array:
    ...
    rms = jnp.sqrt(jnp.mean(x_fp32 ** 2, axis=axis, keepdims=True) + eps)
...
def forward(
    ...
    logits = jnp.einsum(
        "btd,dv->btv", x, weights.lm_head, preferred_element_type=jnp.float32 # prevent bfloat16 logit collapse
    )
